...
import type { AnchorBlindCause, AnchorWatch } from '$entities/anchor';
...
// A watch that has gone blind gets the same first-class strip as a drag: a client watch without
// GPS cannot detect a drag and a server watch without its stream cannot deliver one here, which
// the sleeping crew must be able to acknowledge or act on. A drag outranks either for the title,
// and each condition carries its own acknowledged state (the drag acknowledge is server-graded,
// the blind one per episode).
const BLIND_TITLES: Record<AnchorBlindCause, string> = {
  'fix-lost': 'Anchor watch: no GPS',
  'link-lost': 'Anchor watch: link down',
};
const blindCause = $derived(anchor.blindCause);
const acked = $derived(anchor.dragging ? anchor.acknowledged : anchor.blindAcknowledged);
</script>

{#if anchor.dragging || blindCause !== undefined}
  ...
  <aside class="bottom-strip bottom-strip--alarm" class:is-ack={acked} aria-label="Anchor alarm">
    <div class="head">
      <span class="title">
        {anchor.dragging ? 'Anchor dragging' : blindCause ? BLIND_TITLES[blindCause] : ''}
      </span>
      {#if acked}
        <span class="note ack-tag">Acknowledged</span>
      {:else}
        <div class="actions actions--safety">
          <!-- Before the blind tone arms there is nothing to acknowledge, and a tap here must
               not silently disarm the alarm the grace is still counting toward. -->
          <button
            type="button"
            class="ack"
            disabled={!anchor.dragging && !anchor.blindAlarm}
